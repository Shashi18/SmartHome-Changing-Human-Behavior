...
class HumanModel:
    ...
    def decay(self, x, eps):
        ...
        q = 7*x
        self.epsilon = max(0.001, math.exp(-5.5*x/eps))
    ...
    def getAction(self, i, s):
        ...
        if random.random() < self.epsilon:
            ...
            self.logA.append("".join(str(var).ljust(6) for var in [i, s, a, Q, valid_actions, self.Qr[a, s], self.time.copy(), 'NR']))
            if i==self.root: self.taskStack.append(a)
            return a
    ...
    def mapDown(self, action, index):
        if action in [0, 1, 2]:
            ...
            if action == 0:
                ...
                return taskState.index(action << 40 | (state & 0xff) << 20 | self.initTH[0] << 10 | self.initTH[1])
    ...
    def trainPMV(self, i, s, a):
        ...
        for episode in range(1):
            ...
            while not self.isTerminal(i):
                ...
                self.Qc[i, s, a] = (1-self.alpha)*self.Qc[i, s, a] + alpha*0.9*(self.Qr[a_, s] + self.Qc[i, s, a_])
    ...
    async def MAXQ_HO(self, i, s, task):
        ...
        if self.isPrimitive(i):
            ...
            while not self.isTerminal(i):
                ...
                childSeq = loop.run_until_complete(asyncio.gather(self.MAXQ_HO(a, s, i)))[0]
                s_ = self.nState[i] if i in [0, 1, 2, 3, 4, 5] else self.rootState(a)
                a_ = self.argmax(i, s_)
                self.QrCopy = self.Qr.copy()
                if a == self.cont:
                    continueStates = childSeq + continueStates
                    s = s_
                    if self.time[i] == 39:
                        continueStates = childSeq + continueStates
                    ...
                    if self.flag == 1:
                        self.logB.append((i, a, childSeq))
                    self.update(0, continueStates, i, s_, a_) 
                    visitedStates = continueStates + visitedStates
                    s = s_
                N = 1
